...
public class MaxFilter implements Filter {
	...
	public void apply() {
		...
		// Found Convolution pseudocode here: https://www.cs.auckland.ac.nz/courses/compsci373s1c/PatricesLectures/Image%20Filtering.pdf
	        ...
	        int maxValue = 0; // the variable to replace with the maximum value in the pixel neighbourhood
	            ...
	            for (int x = 0; x < width; x++) {
	               ...
	               for(int j = -kernelW; j <= kernelW ; j++) {
	            	   for(int k = -kernelH; k <= kernelH; k++) {
	            		   ...
	            		   // if the value of the pixel at the position is GREATER than 'maxValue'
	            		   if ( ((inputImage.argb[kernelPos] >> 16) & 0xFF) > maxValue) {
	            			 //then maxValue is updated(assigned) with this GREATER value
	            			   maxValue = ((inputImage.argb[kernelPos] >> 16) & 0xFF);
	            		   }
	            	   }
	               }
	               outputImage.argb[pos] = (0xFF<<24) | (maxValue<<16) | (maxValue<<8) | maxValue;
	              maxValue = 0; // need to update 'maxValue' to 0 again after writing back the values or the image is white
	            }
	        }
	    }
	}