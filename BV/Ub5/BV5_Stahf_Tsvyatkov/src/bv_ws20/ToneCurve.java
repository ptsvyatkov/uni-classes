...
public class ToneCurve {
	...
	private void updateTable() {
		for(int i = 0; i<grayTable.length; i++) {
			...
			double grayIn = i+brightness; // applying brightness first

		//	grayIn = limitRGB((int)grayIn);

			grayIn = contrast * ( grayIn - 128 ) + 128; // applying contrast second (Prof.Barthel slides for formula)
			
			grayIn = limitRGB((int) grayIn);
			
			int grayOut = (int) ((255* (Math.pow(grayIn, 1/gamma))) / (Math.pow(255, 1/gamma))); // then applying gamma ( slide 18 )
