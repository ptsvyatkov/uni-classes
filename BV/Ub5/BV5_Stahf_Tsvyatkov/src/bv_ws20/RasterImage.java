...
public class RasterImage {
	...
	public void convertToGray() {
		// Loops are running through the argb-array
		for(int x = 0; x<width; x++) {
			for(int y = 0; y<height; y++) {
				...
				int gray = (pixel >> 16) & 0xff; // we only need one value since the image already is grayscaled
	...
	public void applyToneCurve(ToneCurve curve) {
		for(int x = 0; x<width; x++) {
			for(int y = 0; y<height; y++) {
				...
				int gray = curve.mappedGray(grayscale); // appliyng the mapped gray value from the table
